...
/// - Decimals must have exactly n digits after the dot for Decimal(m, n)
/// - Integers are by default BigInts (i64)
/// - Other integer types must have _i8, _i16, _i32 suffix
...
pub fn parse_literal_value(input: &str) -> Result<LiteralValue, ParamParseError> {
    ...
    if trimmed.len() >= 2 {
        ...
        let last = trimmed.chars().rev().next().unwrap();
    ...
    if let Ok(val) = trimmed.parse::<i64>() {
        return Ok(LiteralValue::BigInt(val));
    }
...
fn parse_decimal75(input: &str) -> Result<LiteralValue, ParamParseError> {
    // For now, parse as a simple decimal without enforcing precision rules
    // The caller will need to specify Decimal(m, n) separately
    // This is a simplified implementation that assumes the format is correct

    let parts: Vec<&str> = input.split('.').collect();
    if parts.len() != 2 {
        return Err(ParamParseError::InvalidDecimal {
            value: input.to_string(),
        });
    }

    let scale = parts[1].len() as i8;

    // Remove the decimal point and parse as integer
    let without_dot = format!("{}{}", parts[0], parts[1]);
    let value = without_dot
        .parse::<i128>()
        .map_err(|_| ParamParseError::InvalidDecimal {
            value: input.to_string(),
        })?;

    // Determine precision (total number of significant digits)
    let precision = without_dot.trim_start_matches('-').len() as u8;

    // Convert i128 to I256 (from proof-of-sql)
    let i256_value = I256::from(value);

    let precision_obj = Precision::new(precision)
        .map_err(|_| ParamParseError::InvalidPrecision { value: precision })?;
