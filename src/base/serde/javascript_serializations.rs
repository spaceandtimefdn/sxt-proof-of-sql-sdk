...
const SERIALIZATION_FAILED_MESSAGE: &str =
    "{\"verificationStatus\":\"Failure\",\"error\":\"failed to serialize result\"}";
...
> {
    let query_results: QueryResultsResponse =
        serde_json::from_str(&query_results_json).map_err(|err| Failure {
            error: format!("Error deserializing query results: {}", err),
        })?;
    let valid_attestors = valid_attestors
        .iter()
        .map(|attestor| {
            address_from_hex(attestor).map_err(|err| Failure {
                error: format!("Error deserializing attestors: {}", err),
            })
        })
...
mod tests {
    ...
    fn confirm_last_ditch_error_follows_serialization_correctly() {
        let res =
            serde_json::to_string(&VerificationStatus::<JSFriendlyColumn>::Failure(Failure {
